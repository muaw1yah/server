...
#include <limits.h>        // properties of the various variable types
#include <math.h>          // mathematical functions
#include <signal.h>        // for signals reported during a program execution
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if(tokens[0] == NULL || strcmp(tokens[0], "GET") != 0)
            {
                error(405);
                continue;
            }
            ...
            if(tokens[2] == NULL || strcmp("HTTP/1.1", tokens[2]) != 0)
            {
                error(501);
                continue;
            }
            ...
            char* start = strchrnul(tokens[1], '?');
            char* end = strchr(tokens[1], '\0');
            int querySize = (end - start < 1) ? 1 : end - start;
            char query[querySize];
            memcpy(query, (strncmp(start, "\0", 1) == 0) ? start : start + 1, querySize);
            
            start = strchrnul(tokens[1], '?');
            char absPath[start - tokens[1] + 1];
            strncpy(absPath, tokens[1], start - tokens[1]);
            absPath[start -  tokens[1]] = '\0';
            
            // concatenate root and absolute-path
            char path[sizeof(root) + sizeof(absPath) + 1];
            strcpy(path, root);
            strcat(path, absPath);
            ...
            char* pathExtension = strchr(absPath, '.');
            if(pathExtension == NULL)
            {
                error(501);
                continue;
            }          
            end = strchr(absPath, '\0');
            char extension[end - pathExtension];
            memcpy(extension, pathExtension+1, end-pathExtension);
